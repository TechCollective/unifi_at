...
#		last_activity = datetime.strptime(ticket[0]['lastActivityDate'],'%Y-%m-%dT%H:%M:%S')
				...
				for device in c.devices:
					...
					if device['adopted'] != False:
						...
						if 'name' in device.keys():
							name = device['name']
							udf.append({'name': 'Name', 'value': name})
							udf.append({'name': 'AEM_Description', 'value': name})
						else:
...
#						udf.append({'name': 'AEM_Model', 'value': device['model']})
#						udf.append({'name': 'Mac Address', 'value': device['mac']})
#						udf.append({'name': 'AEM_Manufacturer', 'value': "UniFi"})
#						udf.append({'name': 'IP Address', 'value': device['ip']})

#						if device['state'] == 1:
#							last_seen = {'name': 'UniFi Last Seen', 'value': datetime.utcnow().strftime("%Y-%m-%d")}
#							udf.append(last_seen)
...
#							if len(alerts) > 0 :
#								check_alerts(alerts, device['mac'], cid, return_value['itemId'])
