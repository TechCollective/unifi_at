...
at = atSite(config.atHost, config.atUsername,
...
def updateATCI(result, ci):
	last_seen = str(datetime.datetime.fromtimestamp(
		int(result[0]['last_seen'])).isoformat()) + ".0000000"
	udf = [{'name': 'UniFi Last Seen', 'value': last_seen}]

	if 'first_seen' in result[0]:
		first_seen = str(datetime.datetime.fromtimestamp(
			int(result[0]['first_seen'])).isoformat()) + ".0000000"
...
def process_ci(company):
	ci_filter_field = "{'op': 'and','items': [{'op':'eq','field':'isActive','value': '1'},	{'op':'eq','field':'companyID','value': '" + str(
		company['id']) + "' },   {'op':'noteq','field':'productID','value': '" + str(ignoreProduct) + "'} ] }"
				...
				try:
					...
					if ci['rmmDeviceAuditHostname'] is not None:
						if ci['rmmDeviceAuditDescription'] is not None:
							if len(macs) == 1:
								alias = ci['rmmDeviceAuditHostname'] + \
									" - " + ci['rmmDeviceAuditDescription']
							else:
								alias = ci['rmmDeviceAuditHostname'] + \
									"(" + str(count) + ") - " + ci['rmmDeviceAuditDescription']
						else:
...
def process_sites(unifi_field):
	company_filter_field = "{'op':'eq','field':'isActive','value': '1'},{'op':'noteq','field':'" + \
		unifi_field + "','udf': 'true','value': 'None' }"
